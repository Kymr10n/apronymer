...
fn test_performance_comparison() {
    ...
    println!("Limited results (20 max): {} apronyms in {:?}", limited_results.len(), limited_duration);
    println!("Unlimited results: {} apronyms in {:?}", unlimited_results.len(), unlimited_duration);
    
    // The limited version should be faster when there are many results
    assert!(limited_results.len() <= 20, "Limited results should not exceed the limit");
    assert!(!limited_results.is_empty(), "Should find at least some results");
}
...
fn test_early_termination_effectiveness() {
    ...
    println!("Small limit (5): {} apronyms in {:?}", small_limit_results.len(), small_limit_duration);
    println!("Large limit (50): {} apronyms in {:?}", large_limit_results.len(), large_limit_duration);
    
    assert!(small_limit_results.len() <= 5, "Small limit should be respected");
    assert!(large_limit_results.len() <= 50, "Large limit should be respected");
    
    // With early termination, small limit should be significantly faster
    if large_limit_results.len() > 20 {
        assert!(small_limit_duration <= large_limit_duration, "Small limit should be faster or equal");
    }
}
