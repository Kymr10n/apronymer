...
use crate::generator::generate_apronyms; // Optimized generation logic
use crate::rate_limiter::{RateLimiter, RateLimiterStats};
use crate::validator::validate_generate_request; // Request validation // Rate limiter
...
pub async fn generate(Json(payload): Json<GenerateRequest>) -> impl IntoResponse {
    tracing::info!(
        "Handling /generate: terms={:?}, frag_len={}, min_len={}, max_len={}",
        payload.terms,
        payload.frag_len,
        payload.min_len,
        payload.max_len
    ...
    if let Err(err) = validate_generate_request(&payload) {
        tracing::warn!(
            "Validation failed for /generate: terms={:?}, frag_len={}, min_len={}, max_len={}",
            payload.terms,
            payload.frag_len,
            payload.min_len,
            payload.max_len
    ...
    // Use optimized generation with default result limit
    let results = generate_apronyms(
        payload.terms,
        payload.frag_len,
        payload.min_len,
        payload.max_len,
...
pub fn routes() -> Router {
    Router::new().route("/generate", post(generate)) // POST /api/generate
}
