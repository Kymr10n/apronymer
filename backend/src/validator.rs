...
pub fn validate_generate_request(payload: &GenerateRequest) -> Result<(), Response> {
    tracing::info!(
        "Validating generate request: terms={:?}, frag_len={}, min_len={}, max_len={}",
        payload.terms,
        payload.frag_len,
        payload.min_len,
        payload.max_len
    ...
    if payload.terms.len() < 3 {
        return Err((
            ...
            "Please enter at least three valid terms.",
        )
            .into_response());
    }
    ...
    if payload.terms.len() > 10 {
        return Err((
            StatusCode::BAD_REQUEST,
            "Please enter no more than 10 terms.",
        )
            .into_response());
    }
    ...
    if unique_terms.len() != payload.terms.len() {
        return Err((
            StatusCode::BAD_REQUEST,
            "Terms must be unique — please remove duplicates.",
        )
            .into_response());
    }
    ...
    if payload.min_len == 0 || payload.max_len == 0 {
        return Err((
            StatusCode::BAD_REQUEST,
            "Min Length and Max Length must both be at least 1.",
        )
            .into_response());
    }
    if payload.min_len > 10 || payload.max_len > 10 {
        return Err((
            StatusCode::BAD_REQUEST,
            "Min Length and Max Length must not exceed 10.",
        )
            .into_response());
    }
    ...
    if payload.max_len > payload.terms.len() {
        return Err((
            StatusCode::BAD_REQUEST,
            "Max Length cannot exceed the number of terms provided.",
        )
            .into_response());
    }
    ...
    if payload.frag_len < 1 || payload.frag_len > 3 {
        return Err((
            StatusCode::BAD_REQUEST,
            "Fragment Length must be between 1 and 3.",
        )
            .into_response());
    }

    // Check that all terms have at least frag_len characters
    let terms_too_short: Vec<_> = payload
        .terms
        .iter()
    ...
    if !terms_too_short.is_empty() {
        return Err((
            StatusCode::BAD_REQUEST,
            "All terms must have at least as many characters as the specified Fragment Length.",
        )
            .into_response());
    }
    ...
    if payload
        .terms
        .iter()
        .any(|t| !t.chars().all(|c| c.is_alphabetic()))
    {
        return Err((
            StatusCode::BAD_REQUEST,
            "Terms must only contain letters (A-Z).",
        )
            .into_response());
    }
    ...
    if !payload
        .terms
        .iter()
        .any(|t| matches!(t.chars().next(), Some(c) if "aeiouAEIOU".contains(c)))
    {
        return Err((
            StatusCode::BAD_REQUEST,
            "At least one term must start with a vowel (A, E, I, O, U).",
        )
            .into_response());
    }
    Ok(())
