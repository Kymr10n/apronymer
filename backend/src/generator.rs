...
pub fn generate_apronyms_with_limit(
    ...
    let total_combinations = permutations
        .iter()
        .map(|perm| frag_len.pow(perm.len() as u32))
        .sum::<usize>();
    tracing::info!(
        "Processing {} total fragment combinations",
        total_combinations
    );
    ...
    tracing::info!(
        "Generated {} valid apronyms (limit: {})",
        matches.len(),
        max_results
    );
    tracing::debug!(
        "Valid apronyms: {:?}",
        matches.iter().map(|a| &a.text).collect::<Vec<_>>()
    );
    ...
    let all_matches: Vec<Vec<Apronym>> = permutations
        ...
        .filter_map(|(perm_idx, perm)| {
            ...
            if total_combinations > MAX_COMBINATIONS_PER_PERMUTATION {
                tracing::warn!(
                    "Skipping permutation with {} combinations (exceeds safety limit of {})",
                    total_combinations,
                    MAX_COMBINATIONS_PER_PERMUTATION
                );
                return None;
            }

            // Process this permutation and collect matches
            let perm_matches = process_permutation_with_limit(
                terms,
                perm,
                frag_len,
                total_combinations,
            ...
            if !perm_matches.is_empty() {
                let new_count =
                    found_count.fetch_add(perm_matches.len(), std::sync::atomic::Ordering::Relaxed);
                tracing::trace!(
                    "Found {} matches for permutation {:?} (total: {})",
                    perm_matches.len(),
                    perm,
                    new_count + perm_matches.len()
                );
                Some(perm_matches)
            } else {
                None
...
fn generate_sequential(
    ...
    for (perm_idx, perm) in permutations.iter().enumerate() {
        ...
        for i in 0..total_combinations {
            ...
            tracing::trace!(
                "Generated candidate: '{}' from terms {:?}",
                apronym.text,
                apronym.terms
            );
            ...
            if is_valid_word(&apronym.text) {
                tracing::trace!(
                    "Found valid apronym: '{}' from terms {:?}",
                    apronym.text,
                    apronym.terms
                );
...
fn permutate(term_count: usize, min_len: usize, max_len: usize) -> Vec<Vec<usize>> {
    tracing::debug!(
        "Generating permutations: {} terms, lengths {}-{}",
        term_count,
        min_len,
        max_len
    );
